...
describe('computeLocations', () => {
  ...
  it('shares the live condition for regionally plausible coastal/deltaic locations', () => {
    const result = computeLocations({ temperature: 30, condition: 'Showers', conditionCode: 'showers' })
    expect(result.find(l => l.name === 'Digha Beach')?.conditionCode).toBe('showers')
    expect(result.find(l => l.name === 'Sundarbans')?.conditionCode).toBe('showers')
  })

  it('never shows a Kolkata thunderstorm/storm condition for the distant Darjeeling hill station', () => {
    const result = computeLocations({ temperature: 32, condition: 'Thunderstorm', conditionCode: 'thunderstorm' })
    const darjeeling = result.find(l => l.name === 'Darjeeling')!
