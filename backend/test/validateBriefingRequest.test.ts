...
describe('validateBriefingRequest', () => {
  it('accepts a full valid request', () => {
    const result = validateBriefingRequest({ persona: 'outdoor', activity: 'walking', sensitivity: 'normal', location: 'Kolkata' })
  ...
  it('rejects an invalid sensitivity', () => {
    expect(() => validateBriefingRequest({ persona: 'general', sensitivity: 'extreme' })).toThrow()
  })
  ...
  it('rejects an oversized activity string', () => {
    expect(() => validateBriefingRequest({ persona: 'general', activity: 'a'.repeat(200) })).toThrow()
  })

  it('rejects a non-string location', () => {
    expect(() => validateBriefingRequest({ persona: 'general', location: 12345 })).toThrow()
  })
})
