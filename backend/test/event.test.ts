...
describe('computeEvent', () => {
  it('does not fabricate a real festival/event name', () => {
    const daily = buildDaily([10, 10, 10, 10, 10, 10, 10], [30, 30, 30, 30, 30, 30, 30])
    const result = computeEvent({ daily, currentDate: new Date('2026-08-28T06:00:00Z'), month: 7 })
  ...
  it('reports High Rain for a very wet week', () => {
    const daily = buildDaily([80, 85, 90, 88, 92, 87, 90], [28, 28, 28, 28, 28, 28, 28])
    const result = computeEvent({ daily, currentDate: new Date('2026-08-28T06:00:00Z'), month: 7 })
  ...
  it('is deterministic for the same date and forecast', () => {
    const daily = buildDaily([10, 20, 30, 10, 10, 10, 10], [30, 31, 29, 30, 30, 30, 30])
  ...
  it('sets expectedSeason based on month', () => {
    const daily = buildDaily([10, 10, 10, 10, 10, 10, 10], [30, 30, 30, 30, 30, 30, 30])
    const monsoonResult = computeEvent({ daily, currentDate: new Date('2026-08-28T06:00:00Z'), month: 7 })
    const winterResult = computeEvent({ daily, currentDate: new Date('2026-08-28T06:00:00Z'), month: 0 })
