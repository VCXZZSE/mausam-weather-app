...
import {
  ...
  resolveAirQuality,
} from "../src/aqi/resolveAirQuality.js"
  ...
  it("skips CPCB entirely (no network call) when no API key is configured", async () => {
    const fetchSpy = vi.fn(async (input: string | URL) => {
      ...
      if (url.includes("data.gov.in"))
        throw new Error("should not call CPCB without a key")
    ...
    const result = await resolveAirQuality(
      env,
      caches,
      KOLKATA,
      "2026-09-05T05:00",
      ...
      vi.fn(async (input: string | URL) => {
        ...
        if (url.includes("data.gov.in"))
          return {
            ok: false,
            status: 403,
            json: async () => ({ error: "Key not authorised" }),
          }
        return { ok: true, json: async () => openMeteoAqiBody() }
      }),
    )
