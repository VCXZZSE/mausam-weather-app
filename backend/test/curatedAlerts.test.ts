...
describe("computeCuratedAlerts", () => {
  ...
  it("raises a monsoon flooding risk alert only during monsoon months with very high rain chance", () => {
    const monsoon = computeCuratedAlerts({
      ...BASE,
      conditionCode: "rain",
      rainChanceToday: 85,
      month: 6,
    })
    const nonMonsoon = computeCuratedAlerts({
      ...BASE,
      conditionCode: "rain",
      rainChanceToday: 85,
      month: 0,
    ...
    expect(nonMonsoon.some((a) => a.title.includes("Monsoon Flooding"))).toBe(
      false,
    )
  })
  ...
  it("orders alerts most severe first", () => {
    ...
    expect(levels.indexOf("Red")).toBeLessThanOrEqual(
      levels.indexOf("Yellow") === -1 ? Infinity : levels.indexOf("Yellow"),
    )
  })

  it("is deterministic for identical inputs", () => {
    const first = computeCuratedAlerts({
      ...BASE,
      conditionCode: "thunderstorm",
    })
    const second = computeCuratedAlerts({
      ...BASE,
      conditionCode: "thunderstorm",
    })
