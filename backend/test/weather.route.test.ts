...
function stubFetchByUrl(handlers: {
  forecast?: () => unknown
  airQuality?: () => unknown
  forecastFails?: boolean
  airQualityFails?: boolean
}) {
  vi.stubGlobal(
    "fetch",
    vi.fn(async (input: string | URL) => {
      ...
      return {
        ok: true,
        json: async () => (handlers.forecast ?? openMeteoForecastFixture)(),
      }
    }),
  )
}
...
describe("GET /api/weather", () => {
  ...
  it("never returns one location's cached weather for a different location", async () => {
    ...
    const shimla = await app.inject({
      method: "GET",
      url: "/api/weather?latitude=31.1048&longitude=77.1734",
    })
