...
describe('normalizeCpcbAirQuality', () => {
  it('computes a sub-index from a single reported pollutant using the CPCB breakpoint table', () => {
    ...
    const result = normalizeCpcbAirQuality([record({ pollutant_id: 'PM2.5', pollutant_avg: '65' })], KOLKATA, 50)
  ...
  it('selects the nearest station within range and reports its name/distance', () => {
    const near = record({ station: 'Near Station', latitude: '22.58', longitude: '88.37' })
    const far = record({ station: 'Far Station', latitude: '28.6', longitude: '77.2' })
  ...
  it('returns undefined when no station is within the configured max distance', () => {
    const farOnly = record({ station: 'Delhi Station', latitude: '28.6139', longitude: '77.2090' })
  ...
  it('categorizes correctly at category boundaries (50/51, 100/101, etc.)', () => {
    ...
    const good = normalizeCpcbAirQuality([record({ pollutant_id: 'PM10', pollutant_avg: '50' })], KOLKATA, 50)
    const satisfactory = normalizeCpcbAirQuality([record({ pollutant_id: 'PM10', pollutant_avg: '51' })], KOLKATA, 50)
  ...
  it('groups multiple pollutant records for the same station together rather than treating them as separate stations', () => {
    const records = [
      record({ station: 'Station A', pollutant_id: 'PM2.5', pollutant_avg: '40' }),
      record({ station: 'Station A', pollutant_id: 'PM10', pollutant_avg: '60' }),
    ]
    const result = normalizeCpcbAirQuality(records, KOLKATA, 50)
