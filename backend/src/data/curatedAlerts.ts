 ...
 */
export function computeCuratedAlerts(
  input: CuratedAlertInput,
): DashboardWeatherData["alerts"] {
  const alerts: AlertTemplate[] = []

  if (
    input.conditionCode === "thunderstorm" ||
    input.conditionCode === "storm"
  ) {
    alerts.push(
      buildAlert(
        "Red",
    ...
    MONSOON_MONTHS.has(input.month) &&
    input.rainChanceToday >= 80 &&
  ...
  if (input.windSpeed >= 40) {
    alerts.push(
      buildAlert(
        ...
        `Wind gusts near ${Math.round(input.windSpeed)} km/h expected. Secure loose outdoor items.`,
      ),
    )
  }
