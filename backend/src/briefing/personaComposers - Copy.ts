...
function commuter(ctx: PersonaContext, topRisk: TopRisk): ComposedBriefing {
  switch (topRisk?.type) {
    case "thunderstorm":
      return {
        summary:
          "Thunderstorms are expected, which can disrupt travel and pose safety risks.",
        recommendation:
          "Allow extra travel time and avoid exposed routes until storms pass.",
        actions: [
          "Avoid open or exposed routes during thunderstorms",
          "Allow extra travel time today",
        ],
      }
    case "heat":
      return {
        ...
        recommendation:
          "Stay hydrated and avoid unnecessary time outdoors while commuting.",
        actions: [
          "Carry water during your commute",
          "Avoid waiting outdoors longer than necessary",
        ],
      }
    case "rain":
      return {
    ...
    case "wind":
      return {
        ...
        recommendation:
          "Use caution on exposed roads or bridges during your commute.",
        actions: [
          "Use caution on exposed routes",
          "Secure loose belongings while traveling",
        ],
      }
    case "uv":
      return {
        ...
        recommendation:
          "Commute conditions are otherwise favorable; use sun protection if walking.",
    ...
    default:
      return {
        ...
        recommendation:
          "No significant weather disruption expected for your commute.",
...
function outdoor(ctx: PersonaContext, topRisk: TopRisk): ComposedBriefing {
  switch (topRisk?.type) {
    case "thunderstorm":
      return {
        summary:
          "Thunderstorms are expected, which makes outdoor plans risky today.",
        recommendation: "Avoid outdoor activity until thunderstorms clear.",
        actions: [
          "Postpone outdoor plans while thunderstorms are active",
    ...
    case "uv":
      return {
        ...
        actions: [
          "Wear sunscreen and sunglasses if outside",
          "Avoid extended midday sun exposure",
        ],
      }
    case "wind":
      return {
        ...
        recommendation:
          "Outdoor activity is fine, but avoid exposed high ground.",
...
function student(ctx: PersonaContext, topRisk: TopRisk): ComposedBriefing {
  switch (topRisk?.type) {
    case "thunderstorm":
      return {
        summary:
          "Thunderstorms are expected, so travel to and from school/college carries risk.",
        recommendation: "Avoid exposed travel while thunderstorms are active.",
        actions: [
          "Avoid exposed travel during thunderstorms",
          "Keep a rain layer or shelter plan ready",
        ],
      }
    case "heat":
      return {
        ...
        recommendation:
          "Stay hydrated and rest when possible during peak heat.",
        actions: [
          "Carry water through the day",
    ...
    default:
      return {
        summary:
          "Conditions look comfortable for commuting and studying outdoors between classes.",
...
function health(ctx: PersonaContext, topRisk: TopRisk): ComposedBriefing {
  switch (topRisk?.type) {
    case "thunderstorm":
      return {
        summary:
          "Thunderstorms are expected, which is a safety concern for anyone outdoors.",
        recommendation: "Stay indoors while thunderstorms are active.",
        actions: [
          "Stay indoors during thunderstorms",
          "Avoid open areas and tall isolated structures",
        ],
      }
    case "aqi":
      return {
        summary: `India AQI is ${ctx.aqiIndex} (${ctx.aqiLabel ?? "elevated"}) — this can affect sensitive individuals.`,
        recommendation:
          "Reduce prolonged outdoor exposure, especially with respiratory sensitivities.",
        actions: [
          "Limit prolonged outdoor exertion",
          "Consider a mask outdoors if sensitive to air quality",
        ],
      }
    case "heat":
      return {
        ...
        recommendation:
          "Prioritize hydration and cooling; avoid peak-heat exertion.",
        actions: [
    ...
    case "uv":
      return {
        ...
        actions: [
          ...
          "Wear sunglasses and a hat if outdoors",
        ],
      }
    case "rain":
      return {
    ...
    default:
      return {
        ...
        recommendation:
          "No specific precautions needed based on current conditions.",
        actions: [
          "Conditions are favorable — no specific weather precautions needed",
        ],
      }
  }
}
...
const COMPOSERS: Record<Persona, (
  ctx: PersonaContext,
  topRisk: TopRisk,
) => ComposedBriefing> = {
  commuter,
...
export function composePersonaBriefing(
