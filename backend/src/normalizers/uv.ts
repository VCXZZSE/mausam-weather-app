...
const UV_CATEGORIES: Array<{ max: number; label: string; recommendation: string }> = [
  { max: 2, label: 'Low', recommendation: 'No protection needed for most people' },
  { max: 5, label: 'Moderate', recommendation: 'Use SPF 30+ if outdoors for long periods' },
  { max: 7, label: 'High', recommendation: 'Use SPF 30+' },
  { max: 10, label: 'Very High', recommendation: 'Use SPF 50+ · Seek shade at midday' },
  { max: Infinity, label: 'Extreme', recommendation: 'Avoid midday sun · SPF 50+ mandatory' },
]

function categorize(index: number) {
  return UV_CATEGORIES.find(category => index <= category.max) ?? UV_CATEGORIES[UV_CATEGORIES.length - 1]
}
