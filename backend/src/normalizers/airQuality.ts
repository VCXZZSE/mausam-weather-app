...
function categorize(index: number): { label: string; icon: string } {
  return US_AQI_CATEGORIES.find(category => index <= category.max) ?? US_AQI_CATEGORIES[US_AQI_CATEGORIES.length - 1]
}
...
export function normalizeAirQuality(
  ...
  return {
    ...
    pollutants: [
      ...
      { label: 'O₃', value: Math.round(data.hourly.ozone[index] ?? 0), scaleMax: 120, unit: 'µg/m³', color: '#60a5fa' },
      { label: 'NO₂', value: Math.round(data.hourly.nitrogen_dioxide[index] ?? 0), scaleMax: 80, unit: 'µg/m³', color: '#a78bfa' },
    ],
  }
}
