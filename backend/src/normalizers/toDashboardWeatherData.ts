...
)// Pre-resolved from India's CPCB feed by the caller. This normalizer no
...
: WeatherPayload {
  ...
  const heroVariant = resolveHeroVariant(conditionCode, condition, isDay)
  ...
  const currentInstant = toLocationInstant(
    data.current_weather.time,
  ...
  const sunriseInstant = toLocationInstant(
    data.daily.sunrise[0],
    data.utc_offset_seconds,
  )
  const sunsetInstant = toLocationInstant(
    data.daily.sunset[0],
    data.utc_offset_seconds,
  )
  astronomy.sunrise = sunriseInstant.toLocaleTimeString("en-IN", {
    hour: "numeric",
    minute: "2-digit",
    hour12: true,
    timeZone: data.timezone,
  })
  astronomy.sunset = sunsetInstant.toLocaleTimeString("en-IN", {
    hour: "numeric",
    minute: "2-digit",
    hour12: true,
    timeZone: data.timezone,
  ...
  const rainfall = computeRainfall({
    ...
    monthLabel: parseLocalCalendarDate(
      data.current_weather.time,
  ...
  const updatedAtLabel = currentInstant.toLocaleTimeString("en-IN", {
    hour: "numeric",
    minute: "2-digit",
    hour12: true,
    timeZone: data.timezone,
