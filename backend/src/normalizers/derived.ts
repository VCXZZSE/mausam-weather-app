...
export function computeComfort(
  ...
  const index = Math.round(
    clamp(100 - temperaturePenalty - humidityPenalty - windPenalty, 0, 100),
  )
  ...
  return {
    ...
    factors: [
      {
        label: "Temperature",
        value: `${Math.round(input.temperature)}°C`,
        percent: Math.round(clamp((input.temperature / 45) * 100, 0, 100)),
        color: "#f59e0b",
      },
      {
        label: "Humidity",
        value: `${Math.round(input.humidity)}%`,
        percent: Math.round(clamp(input.humidity, 0, 100)),
        color: "#60a5fa",
      },
      {
        label: "Wind",
        value: `${Math.round(input.windSpeed)} km/h`,
        percent: Math.round(clamp((input.windSpeed / 60) * 100, 0, 100)),
        color: "#a78bfa",
      },
    ],
  }
...
export function computeRainfall(
  input: RainfallInput,
): Pick<DashboardWeatherData["rainfall"], "chance" | "today" | "unit" | "periodLabel" | "monthLabel"> {
  return {
...
export function computeOverview(
  input: OverviewInput,
): DashboardWeatherData["overview"] {
  const healthValue =
    input.aqiIndex !== undefined
      ? `India AQI ${input.aqiIndex} · UV ${input.uvIndex}`
