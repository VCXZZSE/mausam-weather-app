...
export function computeEvent(input: EventInput): DashboardWeatherData["event"] {
  ...
  const weekendIndices = [daysAway, daysAway + 1].filter(
    (i) => i < input.daily.length,
  )
  const relevantDays =
    weekendIndices.length > 0
      ? weekendIndices.map((i) => input.daily[i])
      : input.daily

  const avgHigh = Math.round(
    relevantDays.reduce((sum, d) => sum + d.high, 0) / relevantDays.length,
  )
  const avgRainChance = Math.round(
    relevantDays.reduce((sum, d) => sum + d.rainChance, 0) /
      relevantDays.length,
  )
