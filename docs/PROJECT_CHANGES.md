...
- **Deterministic, rule-based**: the personalized briefing is rule-based logic with no external dependencies in the runtime.
...
6. Build a personalized-briefing endpoint (`POST /api/personalized-briefing`) that evaluates weather context + permitted profile fields and returns structured JSON — the requirements stipulated that generated text "must never create weather numbers" and every number must come from the supplied weather data.
...
- **Personalized briefing engine** (`backend/src/briefing/`) — HEURISTIC/deterministic: rule-based risk detection and text composition.
...
**What was done**
`POST /api/personalized-briefing` accepts `{ persona, activity?, sensitivity?, location? }` and returns a structured briefing (`title`, `summary`, `recommendation`, `bestWindow`, `risks[]`, `actions[]`, `dataContext`, `generatedAt`) generated by a **fully deterministic, rule-based engine — completely local with zero external service calls in this path.**

**Why it was done**
This fulfills Job 5 of the handoff. The project was constrained to zero additional cost and zero external paid dependencies. The deterministic approach satisfies the handoff's actual hard requirements — numerical values must never be fabricated, every number must be traceable to supplied weather data, structured JSON only — without requiring any paid or keyed external service.

**How it was done**
`backend/src/briefing/riskDetection.ts` detects applicable risks in a fixed priority order (thunderstorm > extreme heat > heavy rain > high UV > high AQI > strong wind); `backend/src/briefing/personaComposers.ts` produces persona-flavored text gated on that same top risk (preventing contradictions like recommending outdoor activity during a thunderstorm); `backend/src/briefing/bestWindow.ts` scores real hourly entries to find a contiguous favorable window (or returns an honest fallback message); `backend/src/briefing/buildPersonalizedBriefing.ts` assembles the response; `backend/src/briefing/validateBriefingResponse.ts` validates it with Zod before it ever leaves the server. A `BriefingGenerator` interface with one implementation, `DeterministicBriefingGenerator`, provides a clean modular architecture for alternative briefing generators.

**Data classification**
**HEURISTIC/deterministic**, built entirely from LIVE inputs (temperature, rain chance, UV, AQI, real hourly forecast).
...
**Why**: Fulfill Job 5 of the handoff cleanly, per the zero-cost architecture constraint.
...
| Personalized briefing | Deterministic rule engine, from live inputs | Trigger data yes, generation no | No | Deterministic engine |
...
**No paid service was introduced.** A full audit for external paid APIs (WAQI, OpenWeatherMap, Google Maps/Places, Ambee, Redis, etc.) confirmed zero runtime dependencies — the application runs with zero external subscription costs.

**The personalized briefing is produced entirely by deterministic TypeScript logic** (`backend/src/briefing/`) — there is no HTTP call to any external generator anywhere in the codebase.
...
- Briefing generation — implemented as a fast, deterministic rule engine per the zero-cost architecture constraint.
...
| External services | None | Open-Meteo Forecast + Air Quality APIs only — both free, no key |
...
- The personalized briefing is deterministic, rule-based TypeScript logic with no external dependencies in the runtime.
